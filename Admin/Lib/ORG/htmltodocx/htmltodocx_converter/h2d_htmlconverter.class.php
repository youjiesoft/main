...
function _htmltodocx_get_style($element, $state) {
	...
	$new_style = array();
	if($element->attr && !empty($element->attr["style"])){//解决文本颜色
		$attr = $element->attr;
		...
		foreach($style as $k => $v){
			...
			if(mb_strpos($colorStyle, "font-family:") === 0){//解决文本字体
				$colorStr = str_replace("font-family:", "", $colorStyle);
				$new_style['name'] = $colorStr;
			}
			if(mb_strpos($colorStyle, "font-size:") === 0){//解决文本字体大小
				$colorStr = str_replace("font-size:", "", $colorStyle);
				$new_style['size'] = str_replace("px", "", $colorStr);
			}
			if(mb_strpos($colorStyle, "line-through") !== false){//解决文本加删除线
				$new_style['strikethrough'] = true;
			}
		}
	}
	if(find_parent_strikethrough($element)){//解决文本加删除线
		$new_style['strikethrough'] = true;
	}
...
function find_parent_strikethrough($element){
	if( ! empty($element)){
		if(mb_strpos($element->parent->attr["style"], "line-through") !== false){
			return true;
		}else{
			find_parent_strikethrough($element->parent);
		}
	}
	return false;
}
...
function htmltodocx_insert_html_recursive(&$phpword_element, $html_dom_array, &$state = array()) {
	...
	foreach ($html_dom_array as $element) {
		...
		$state['current_style']['spacing'] = 360; //设置默认行间距为1.5
		$state['current_style']['name'] = "仿宋_GB2312";
		$state['current_style']['size'] = 12;
		switch ($element->tag) {
			...
			case 'h5':
			case 'h6':
				if(in_array($element->tag, array('p')) && mb_strpos($element->attr["style"], "text-indent") !== false){
					$attr = $element->attr;
					$str = explode(";",$attr["style"]);
					foreach($str as $k => $v){
						if(mb_strpos($v, "text-indent") !== false && mb_strpos($v, "0em") === false && mb_strpos($v, "0px") === false){//加上行缩进
							$state['current_style']['indentFirstLine'] = 420;
							$state['current_style']['indentFirstLineChars'] = 200;
						}
					}
				}
				...
				if (in_array($element->tag, $allowed_children)) {
					...
					$state['textrun']->addText(htmltodocx_clean_text($element->innertext),  $state['current_style'], $state['current_style']);
				}
				unset($state['textrun']);
				if (!$space_after_set) { //默认行后换行
...
// 					$phpword_element->addTextBreak();
			...
			case 'table':
// 				unset($_SESSION["htmltodocx"]["table_rowspan"]);
				if(isset($_SESSION["htmltodocx"]["html_table_index"])){
					unset($_SESSION["htmltodocx"]["html_tr_index"]);
					$_SESSION["htmltodocx"]["html_table_index"]++;
				}else{
					$_SESSION["htmltodocx"]["html_table_index"]=0;
				}
...
// 					$phpword_element->addTextBreak();
			...
			case 'tr':
				unset($_SESSION["htmltodocx"]["html_td_index"]);
				if(isset($_SESSION["htmltodocx"]["html_tr_index"])){
					$_SESSION["htmltodocx"]["html_tr_index"]++;
				}else{
					$_SESSION["htmltodocx"]["html_tr_index"]=0;
				}
			...
			case 'th':
				...
				if (in_array($element->tag, $allowed_children) && $state['table_allowed']) {
					...
					$new_current_style = $state['current_style'];
					if( ! empty($attr["colspan"])){
						$state['current_style']['gridSpan'] = $attr["colspan"];
					}else{
						$attr["colspan"] = 0;
						unset($state['current_style']['gridSpan']);
					}
					
					if(isset($_SESSION["htmltodocx"]["html_td_index"])){
						if(empty($attr["colspan"])){
							$_SESSION["htmltodocx"]["html_td_index"]++;
						}else{
							$_SESSION["htmltodocx"]["html_td_index"] += (int)$attr["colspan"];
						}
					}else{
					...
					if( ! empty($attr["rowspan"])){
						...
						for($i=0;$i<$attr["rowspan"];$i++){
							$rowListTd = $_SESSION["htmltodocx"]["html_td_index"];
							if($attr["colspan"]>0){
								$rowListTd = $_SESSION["htmltodocx"]["html_td_index"]-$attr["colspan"]+1;
							}
							$rowList[] = array(
									"td"=>$rowListTd,
						...
						$state['current_style']['vMerge']="restart";
					}else{
						unset($state['current_style']['vMerge']);
					}
...
// 					$phpword_element->addTextBreak();
			...
			case 'img':
				...
				$_SESSION["htmltodocx_img"][] = array("src"=>$src,"style"=>$state['current_style']);
				break;
