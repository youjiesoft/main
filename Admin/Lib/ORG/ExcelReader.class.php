...
class Spreadsheet_Excel_Reader {
	...
	// =========================================
	function dump($replacedata=array(),$row_numbers=false,$col_letters=false,$sheet=0,$table_class='excel',&$outwidthandheight) {
		...
		for($row=1;$row<=$this->rowcount($sheet);$row++) {
			...
			$style = "height:" . ceil($rowheight*(4/3)) . "px;";
			...
			for($col=1;$col<=$this->colcount($sheet);$col++) {
				...
				if(!$this->sheets[$sheet]['cellsInfo'][$row][$col]['dontprint']) {
					...
					if(($this->colwidth($col,$sheet)*1) >0 ){
						$style.= "width:" . ($this->colwidth($col,$sheet)*1) . "px;";
					}
...
//							$o1=substr($o1[0], 2);
							...
							$outwidthandheight[$o1]['height']=$rowheight*(4/3);
							$outwidthandheight[$o1]['width']=$this->colwidth($col,$sheet)*1;
						}else{
	...
	//test date   by wangcheng
	function getconfigdata($row_numbers=false,$col_letters=false,$sheet=0) {
		$out = array();
		for($row=1;$row<=$this->rowcount($sheet);$row++) {
			for($col=1;$col<=$this->colcount($sheet);$col++) {
				// Account for Rowspans/Colspans
	...
	/**
	 * 获取标准Excel模板中的内容,返回格式为｛｛0=>说明文字,1=>字段名｝｝
	 * @author 屈强@20140612
	 * @param int $startRow	开始行，默认1，从第一行开始
	 * @param int $sheet 工作薄 默认为0
	 */
	function getConfigDumpData($startRow = 1 , $sheet=0){
		$out = array();
		$kk = 0;
		$temp=array();
