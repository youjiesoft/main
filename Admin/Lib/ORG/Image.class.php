...
/*
 * @author yangxi
 * @create date 2013 01 19
 * @remark 增加签名图章生成效果
 */
 	static function drawSpot($im, $color, $thickness = 1 , $type = 0, $borderSize = 5) {
		for($i = 0; $i < $thickness; $i++)
		{
			imagesetpixel($im, rand(0, imagesx($im)), rand(0, imagesy($im)), $color);
		}
	}

	static function drawBorder($im, $color, $thickness = 1)
	{
		$x1 = 0;
		$y1 = 0;
		$x2 = ImageSX($im) - 1;
	...
	function setSignature($id,$string = ''){
		//字体路径
		$fontfile = dirname(__FILE__)."\\"."msyh.ttf";
		if(!$string) {
			$string = $_POST['name'];
		}
		...
		*/
		 ...
		 * 计算文字长度及图片长度
		*/
		$fontBoxArray = imagettfbbox($fontSize, 0, $fontfile, $string);
		if ($fontBoxArray) {
			$fontWidth = $fontBoxArray[2] - $fontBoxArray[6];
			$fontHeight = $fontBoxArray[3] - $fontBoxArray[7];
		} else {
			$fontWidth = 200;
			$fontHeight = 50;
		}
		...
		*/
		...
		$x = ($width - $fontWidth) / 2 - 1;
		$y = ($height - $fontHeight) / 2 + $fontSize + 3;

		//     	$arr['width'] = $width;
		//     	$arr['height'] = $height;
		//     	$arr['font_width'] = $fontWidth;
		//     	$arr['font_height'] = $fontHeight;
		//     	$arr['x'] = $x;
		//     	$arr['y'] = $y;
		// 		dump($arr);exit();
		/*
		...
		*/
		 ...
		 * 开始填充
		 */
		@imagefilledrectangle($im, 0, 0, $width - 1, $height - 1, $white);		  //填充背景色
		self::drawBorder($im, $red, $borderSize);	  							  //加边框
		self::drawSpot($im, $white,300);
		self::drawSpot($im, $red, 20, 1, $borderSize);
		imagettftext($im,$fontSize,0,$x,$y,$red,$fontfile,$string);				  //加文字
		$fieldname = SIGNATURE_PATH."/".$id.'.gif';							      //保存图片路径
		Image::output($im,'gif',$fieldname);									  //保存图片
	}
