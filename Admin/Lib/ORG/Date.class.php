...
// +----------------------------------------------------------------------
// $Id: Date.class.php 2601 2012-01-15 04:59:14Z liu21st $
...
class Date {
    ...
    public function dateDiff($date, $elaps = "d") {
        ...
        //计算天数差,结果增加1天；
        $__DAYSELAPS = (($this->parse($date) - $this->date) / $__SECONDS_IN_A_DAY__ )+1;
        switch ($elaps) {
            case "y"://转换成年
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    public function timeDiff( $time ,$precision=false,$cn_code=true) {
        if(!is_numeric($precision) && !is_bool($precision)) {
            static $_diff = array('y'=>'年','M'=>'个月','d'=>'天','w'=>'周','s'=>'秒','h'=>'小时','m'=>'分钟');
            return $cn_code ? ceil($this->dateDiff($time,$precision)).$_diff[$precision].'前':ceil($this->dateDiff($time,$precision)).$_diff[$precision];
        }
