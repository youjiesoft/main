...
{//类定义开始
    ...
    public $saveRule2 = false; //同时上传时防止saverule 相同时修改文件名唯一
    // 上传文件Hash规则函数名
     ...
     * @access public
     +----------------------------------------------------------
     */
    public function __construct($maxSize='',$allowExts='',$allowTypes='',$savePath=UPLOAD_PATH,$saveRule='',$saveRule2=false)
    {
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    public function upload($savePath ='',$rename="")
    {
        //如果不指定保存文件名，则由系统默认
        if(empty($savePath)) {
        ...
        foreach($files as $key => $file) {
            ...
            if(!empty($file['name'])) {
                ...
                if($rename){
					$file['savename']   = $rename;
                }else{
                	$file['savename']   = $this->getSaveName($file,$key);
                }
     ...
     * @return string
     +----------------------------------------------------------
     */
    private function getSaveName($filename,$key)
    {
        $rule2="";
        if( $this->saveRule2 ){
            $rule2=$key;
        ...
        if($this->autoSub) {
            // 使用子目录保存文件
            $saveName   =  $this->getSubName($filename).'/'.$rule2.$saveName;
        }
        $saveNametrue=$saveName.".".$filename['extension'];

        //判断重复时重命名
        if( file_exists( $filename['savepath'].$saveNametrue) ){
             while( true ) {
                $saveNametrue = $saveName."_".rand(10,99).".".$filename['extension'];
                if( !file_exists(  $filename['savepath'].$saveNametrue ) ) {
                    break;
                }
            }
