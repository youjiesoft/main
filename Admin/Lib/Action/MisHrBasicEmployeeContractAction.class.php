<?php
/**
 * @Title: MisHrBasicEmployeeContractAction
 * @Package package_name
 * @Description: todo(员工合同管理)
 * @author laicaixia
 * @company 重庆特米洛科技有限公司
 * @copyright 本文件归属于重庆特米洛科技有限公司
 * @date 2013-8-27 下午4:52:00
 * @version V1.0
 */
class MisHrBasicEmployeeContractAction extends CommonAction {
	public function index() {
		...
		// 		$map['contracttype']='合同';
		if($_REQUEST['working']){
			//$map['working']=$_REQUEST['working']==1?1:0;
			switch($_REQUEST['working']){
				case 1:
					$map['working']=1;
					$map['contracttype']='合同';
					break;
			...
			$this->assign("working",$_REQUEST['working']);
		}
	...
	/**
	 * @Title: getContractWorkstatus
	 * @Description: todo(在职员工合同与离职员工合同树结构)
	 * @author renling
	 * @date 2013-12-10 下午4:56:31
	 * @throws
	 */
	private  function getContractWorkstatus(){
		...
		$tree[] = array(
				...
				'name' => '员工合同类型',
				'title' => '员工合同类型',
				'rel' => "mishrbasicemployeecontract",
				'target' => 'ajax',
		...
		$tree[] = array(
				...
				'rel' => "mishrbasicemployeecontract",
				'target' => 'ajax',
				'url' => "__URL__/index/jump/1/working/1",
				'open' => true
		);
		...
		$tree[] = array(
				...
				'rel' => "mishrbasicemployeecontract",
				'target' => 'ajax',
				'url' => "__URL__/index/jump/1/working/3",
				'open' => true
		);
		$tree[] = array(
				'id' => 4,
				...
				'rel' => "mishrbasicemployeecontract",
				'target' => 'ajax',
				'url' => "__URL__/index/jump/1/working/2",
				'open' => true
		);
		$this->assign("tree",json_encode($tree));
	}
	/**
	 * @Description: _before_add(打开页面前置函数)
	 * @author laicaixia
	 * @date 2013-8-28 下午6:33:45
	 */
	...
	/**
	 * @Title: lookupgetfixed
	 * @Description: todo(动态计算工作年限)
	 * @author l
	...
	/**
	 * @Description: lookupmanage(姓名的查找带回)
	 * @author laicaixia
	 * @date 2013-8-28 上午10:55:42
	 */
	public function lookupmanage(){
		//查询在合同中的人
		$hrBasicEmployeeContractModel=D("MisHrBasicEmployeeContract");
		$ids=$hrBasicEmployeeContractModel->field("employeeid")->where("contractstatus=0 and status=1")->select();
		$mapids=array();
		foreach ($ids as $k=>$v) {
			$mapids[]=$v['employeeid'];
		}
		...
		$map = array();
		if($mapids){
			$map['mis_hr_personnel_person_info.id']=array('not in',$mapids);
		}
	...
	/**
	 * @Title: importconfig
	 * @Description: todo(导入合同字段配置)
	 * @author renling
	 * @date 2013-11-25 上午11:40:54
	 * @throws
	 */
	public  function lookupimportconfig(){
	...
	/**
	 * @Title: import
	 * @Description: todo(导入员工合同)
	 * @author renling
	 * @date 2013-12-2 上午11:06:18
	 * @throws
	 */
	public function lookupimport(){
		//订单号可写
			...
			if($filetype=="xls"){
				...
				foreach($dataList as $k=>$v){
					if($i >= 1 && $v['A']){ //标题除外
						...
						$maxid=$maxid+1;
						$orderno = $scnmodel->GetRulesNO('mis_hr_basic_employee_contract',$maxid); //合同编码
						$saveMap['orderno']=$orderno;//合同编码
						$saveMap['name']=$v['B'];//姓名
						$saveMap['sex']=$MisHrBasicEmployeeList['sex'];//性别
						$saveMap['tel']=$MisHrBasicEmployeeList['phone'];//联系电话
						$saveMap['cardid']=$v['C'];//身份证
						$saveMap['familyaddress']=$MisHrBasicEmployeeList['nativeaddress'];//家庭地址
