 ...
 * @version V1.0
 */
class MisSystemPanelRoleAction extends CommonAction {
    /**
    * @Title: _before_index 
    ...
    */
    public function _before_index(){
        $roletype = array(
            ...
            '1' => array(
                'name' => '用户授权',
                'md' => 'MisSystemPanelRole',
            ),
			'2' => array(
                'name' => '部门授权',
                'md' => 'MisSystemPanelDepartmentRole',
            ),
        );
        $roletree = array();
    ...
    */
    public function index(){
		...
		$map['status'] = 1;
		if (!$_REQUEST['md']) {
			$_REQUEST['md'] = 'MisSystemPanelRole';
		}
		if ($_REQUEST['md'] == 'MisSystemPanelRole') {
			$map = $this->_search ();
			$name = $_REQUEST['md'];
			$this->_list ( $name, $map );
			//searchby搜索扩展
		...
		if ($_REQUEST['md'] == 'MisSystemPanelDepartmentRole') {
    ...
    */
    public function getPanelRoleList(){
		...
		$panenmodel = D("MisSystemPanel");
        $map = array();
        $map['status'] = 1;
        $panellist = $panenmodel->where($map)->getField('id,name,modelname,isbasepanel');
		$rolelist = "";
		$map = array();
		$map['status'] = 1;
		if ($_REQUEST['type'] == 1) {
			$rolemodel = D("MisSystemPanelUserRole");
			$map['userid'] = $_REQUEST['partid'];
		} else {
			$rolemodel = D("MisSystemPanelDepartmentRole");
    ...
    */
    public function saveSetPanelRoleUser(){
		...
		if ($_REQUEST['type'] == 1) {
			$model = D("MisSystemPanelUserRole");
			$_POST['userid'] = $_POST['partid'];
		} else {
			$model = D("MisSystemPanelDepartmentRole");
