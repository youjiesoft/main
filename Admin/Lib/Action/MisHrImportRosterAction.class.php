<?php
/**
 * @Title: MisHrImportRosterAction
 * @Package package_name
 * @Description: todo(人力资源- 导入员工花名册)
 * @author renling
 * @company 重庆特米洛科技有限公司
 * @copyright 本文件归属于重庆特米洛科技有限公司
 * @date 2013-11-29 下午3:08:40
 * @version V1.0
 */
class MisHrImportRosterAction extends CommonAction{
	/**
	...
	/**
	 * @Title: import
	 * @Description: todo(导入员工花名册 试用 正式)
	 * @author renling
			...
			if($filetype=="xls"){
				...
				foreach($dataList as $k=>$v){
					...
					if($i >= 1 && $v['B']){
						if($v['C']){
							//判断是否有此身份证数据  有更新
							$list=$MisHrBasicEmployeeModel->where(" status=1  and working=1 and chinaid='".$v['C']."'")->find();
						}
						if($v['T']){
							//查看部门是否存在
						...
						$v['X']=str_replace('.', '-',$v['X']);
						$saveMap['agreetime']=strtotime($v['X']);//签约时间
