...
error:DWZ.ajaxError});},
loadUrl:function(url,data,callback,global){
...
$(this).ajaxUrl({url:url,data:data,callback:callback,type:'post',global:global});},
initUI:function(){
...
// 绑定日期事件
//$("input.js-wdate",$p).click(function(){
//	var $this = $(this);
//	var format = $this.attr("format");
//	var json = {el:$this[0]};
//	if(format){
...
// 绑定日期a标签选择事件
//$("a.js-inputCheckDate",$p).click(function(){
//	var $prev = $(this).prev();
//	// 扩展格式化1
//	var format = $prev.attr("format");
//	var json = {el:$prev[0]};
...
if($(".into_table_new").length)initTableWNEW();
...
//				option_data.push({"id":$(this).find("option").eq(i).attr("value"),"name":$(this).find("option").eq(i).text()});
//			}
//			var input = $('<input type="text">');
//			input.attr("name",$(this).attr("name")).attr("class",$(this).attr("class")).attr("id",$(this).attr("id"));
//			$(this).after(input);
//			input.ajaxComboBox(
//					option_data,
//			    {
...
//	});	
	...
	$('[crop]',nbox).off('click');
	$('[crop]',nbox).on('click',function(){
		var $this = $(this);
		var parentObj = $this.closest('.form_group_lay');
		var title=$this.attr("title")||$this.text();
		var rel=$this.attr("rel")||"_blank";
		var options={};
		var config = {};
		if(parentObj.attr('config')){
			config = $.parseJSON(parentObj.attr('config'));
		}
		options.width =  900;
		options.height = 600;
		options.mask = true;
		...
		options.param={'src':$this.attr('crop'),config:config};
...
this._closeTimer=setTimeout(function(){alertMsgTip.close()},3500);}else{
//$(this._bgId).show();
}
var jCallButs=$(this._boxId).find("[rel=callback]");
for(var i=0;i<buttons.length;i++){
...
$panel.loadUrl(url,{},function(){navTab._loadUrlCallback($panel);});}}else{$(window).trigger("resizeGrid");}},
...
$.pdialog.closeCurrent();
return false;});
}else if(targetType=="navTab"){jGroup.find(":button.close").click(function(){
...
jGroup.attr("inited",1000).find("input[type=text]").filter("[alt]").inputAlert();}}}});})(jQuery);
/////////////////////////////////////////	选项卡结束	///////////////////////////////////
...
desc:$this.attr("desc")||"desc"});});}});})(jQuery);(function($){//by wangcheng add tip
...
$objc.mousemove(function(e){$("#tipsview").css("top",(e.pageY - xOffset-t) + "px").css("left",(e.pageX + yOffset+26) + "px");
$("#tipsview2").css("bottom",-4 + "px").css("left", 20 + "px");
});}})};})(jQuery);(function($){
...
if(json.rel){
    ...
    if(json.refreshtabs&&json.refreshtabs.data!=null){var d = json.refreshtabs.data;
    }else{var d="";
    }
    navTabPageBreak({realnavTab:true,refreshtabs:d},json.rel);
}
...
var d= jQuery.parseJSON( json.data );
var $box =$.pdialog.getCurrent();
if(d){$box.find("table tr").each(function(i){
var obj=$(this);var val=obj.attr("rel");
$.each(d, function(key, v) {if(val==v){obj.remove();}});});}}else{
if(json.redalert==1){
var d= jQuery.parseJSON( json.data );
var $box =$.pdialog.getCurrent();
if(d){
...
if(op.rel){
var $box=$parent.find("#"+op.rel);
var form=_getPagerForm($box,op.data);}else{
var form=_getPagerForm($parent,op.data);}
...
if(data){var obj2=$("<table>"+data+"<table>");
...
$tr.click(function(){
if( cbox>0 ){var cboxchecked=$ftdfisrtcheckbox.is(':checked');
var cboxdisabled=$ftdfisrtcheckbox.is(':disabled');
if(!cboxdisabled){$ftdfisrtcheckbox.attr("checked",!cboxchecked);
if(!cboxchecked){$tr.addClass("checkedbox");
...
function onUploadSuccess(file, data, response) {
	...
	var isCrop = parentObj.attr('category')=='uploadpic'|| parentObj.attr('category')=='picedit' ? true:false;
	// nbm_crop
...
okCall:function(){
ajaxTodo(url,$this.attr("callback"),callbackdata );}});}else{
...
if(!$.isFunction($callback))$callback=eval('('+callback+')');
$callback($tr);}}}//end extend already tr - by qchlian
