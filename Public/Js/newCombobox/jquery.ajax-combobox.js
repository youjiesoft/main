...
$.extend(AjaxComboBox.prototype, /** @lends AjaxComboBox.prototype */ {
	...
	_setOption1st: function(source, option) {
		return $.extend({
			...
			navi_num: 5,
			primary_key: 'id',
			button_img: TP_PUBLIC+'/js/newCombobox/btn.png',
			bind_to: false,
			navi_simple: false,
		...
		switch (this.option.lang) {
			...
			default:
				message = {
					...
					loading_alt : '(loading)',
					page_info   : 'num_page_top - num_page_end of cnt_whole',
					select_ng   : '注意:请从列表中选择。',
					select_ok   : '',
					not_found   : '没有找到',
					ajax_error  : '连接到服务器时发生一个错误。'
				};
				break;
...
//			}
				$(elem.combo_input).removeAttr('name');
	...
	_setInitRecord: function() {
		if($(this.elem.combo_input).attr("init_record")!=undefined)this.option.init_record=$(this.elem.combo_input).attr("init_record");
