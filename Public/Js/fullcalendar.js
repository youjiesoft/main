...
function DayEventRenderer() {
	...
	function daySegHTML(segs) { // also sets seg.left and seg.outerWidth
		...
		for (i=0; i<segCnt; i++) {
			...
			//add by qchlian  chagne current td background
			if(event.tdclassName){
				var d=formatDate(event.start, 'yyyy-MM-dd') ;
				//add by liminggang  change DWZ action scope
				navTab.getCurrentPanel().find("table.fc-border-separate td").each(function(){
					var dd=$(this).attr("data-date");
					if(d==dd){
						$(this).addClass(event.tdclassName);
					}
				});
			}
		}
