function SFNS()
{
	...
	} function _OBS_3(str,password)
	{
		...
		*/
		...
		if (resultStr == "比较基准开始时间") {
			resultStr = "实际开始时间";
		}
		if (resultStr == "比较基准结束时间") {
			resultStr = "实际结束时间";
		}
		if (resultStr == "开始时间") {
			resultStr = "计划开始时间";
	...
	},drawElement: function(element,viewIndex)
	{
		...
		cell.style.cssText="overflow:hidden;white-space:"+whiteSpace+";font-size:0px;border-bottom:1px solid #CDCDCD;border-right:1px solid #CDCDCD;";
		...
		for(var j=0;j<fields.length;j++)
		{
			...
			text.push("border-bottom:1px solid #CDCDCD");
			text.push("border-right:1px solid #CDCDCD");
			...
			{
				//cell.vAlign="top";
				cell.vAlign="middle";//全部上下居中显示
				var div=document.createElement("div");
				_OBS_4(div.style,{
					width:_OBS_1[177],
					position:_OBS_1[178],
					top:'-1px',
					backgroundColor:_OBS_1[74],
					height:"1px",
					fontSize:'0px',
					overflow:_OBS_1[180]
				});
		...
		field.addIcon(function(element,gantt)
			{
				...
				var hrefjson = eval("(" + element.HyperlinkAddress +")");
				link.href = hrefjson.href;
				link.target = hrefjson.target !== undefined ? hrefjson.target : "navTab";
				link.title = hrefjson.title !== undefined ? hrefjson.title : "";
				if (hrefjson.rel !== undefined) {
					link.rel = hrefjson.rel;
				}
				/*
				...
				*/
				...
				if(gantt.setTooltip)
				{
					gantt.setTooltip(img,function(tooltip)
					{
						...
						var str=element.Hyperlink?element.Hyperlink:link.title;
						tooltip.setContent(gantt.container.ownerDocument.createTextNode(str));
						return true;
					});
				}
