...
(function($){
	...
	_consts = {
		...
		folder: {
			...
			DOCC: "text-alt",
			FEND : 'alt',
			//OPENFOLDER:'icon-folder-open-alt',
			//CLOSEFOLDER:'icon-folder-close-alt'
		},
		node: {
	...
	view = {
		...
		appendParentULDom: function(setting, node) {
			...
			view.makeUlHtml(setting, node, html, childHtml.join(''));
			//by wangcheng ��Ĭ�Ϲرսڵ��ʱ�ӽڵ㵯���
			//nObj.append(html.join('')).initUI();
			//修复initUI()对象错误问题 by liminggang
			var ht = $(html.join('')).initUI();
			nObj.append(ht);
		},
		...
		makeDOMNodeNameBefore: function(html, setting, node) {
			...
			html.push("<a rel='",view.makeNodeRel(node),"' id='", node.tId, consts.id.A, "' class='", consts.className.LEVEL, node.level,"' treeNode", consts.id.A," onclick=\"", (node.click || ''),
		...
		makeNodeIcoClass: function(setting, node) {
			var icoCss = ["icon-folder"];
			//var icoCss = [""];
			if (!node.isAjaxing) {
				icoCss[0] = (node.iconSkin ? node.iconSkin + "-" : "") + icoCss[0];
				if (node.isParent) { // 有子级节点
					//$(this).logs('我在点击:'+node.open);
					icoCss.push(node.open ? consts.folder.OPEN : consts.folder.CLOSE);
					icoCss.push(consts.folder.FEND);
					//icoCss.push(node.open ? 'icon-folder-open-alt' : 'icon-folder-close-alt');
					//alert(icoCss);
				} else {
					var icoCss = ["icon-file"];// nbmxkj
					icoCss.push(consts.folder.DOCC);
				}
			}
			return icoCss.join('-');
		},
		...
		removeChildNodes: function(setting, node) {
			...
			if (!setting.data.keep.parent) {
				...
				view.replaceIcoClass(node, tmp_icoObj, consts.folder.DOCC);
		...
		removeNode: function(setting, node) {
			...
			if (!setting.data.keep.parent && childLength == 0) {
				...
				view.replaceIcoClass(parentNode, tmp_icoObj, consts.folder.DOCC);
		...
		replaceIcoClass: function(node, obj, newName) {
			...
			//$(this).logs('这是什么：'+tmpName + ' 这是啥？:'+newName);
			if (tmpName == undefined) return;
			var tmpList = tmpName.split("-");
			if(tmpList[tmpList.length-1] == consts.folder.FEND){
				// 最后一个样式的够成为 FEND 的值
				// 这是为 icon-folder-open-alt[展开],icon-folder-close-alt[关闭] 这种样式写法准备的
				tmpList[tmpList.length-2] = newName;
			}else{
				switch (newName) {
					case consts.folder.OPEN:
					case consts.folder.CLOSE:
					case consts.folder.DOCC:
						//$(this).logs('到这来了?'+consts.folder.DOCC);
						tmpList[tmpList.length-1] = newName;
						break;
				}
			}
			//$(this).logs('我是处理后的数据'+tmpList.toString());
			obj.attr("class", tmpList.join("-"));
		},
		replaceSwitchClass: function(node, obj, newName) {
			if (!obj) return;
			var tmpName = obj.attr("class");
			//$(this).logs('我是点击项的样式:'+tmpName);
			if (tmpName == undefined) return;
	...
	_view = {
		...
		moveNode: function(setting, targetNode, node, moveType, animateFlag, isSilent) {
			...
			if (!setting.data.keep.parent && oldParentNode[childKey].length < 1) {
				...
				view.replaceIcoClass(oldParentNode, tmp_icoObj, consts.folder.DOCC);
				tmp_ulObj.css("display", "none");
