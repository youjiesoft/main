(function($) {
    $.fn.Huploadify = function(opts) {
        var itemTemp = '<div id="${fileID}" class="uploadify-queue-item"><div class="uploadify-info"><span style="display:none;" class="swf_upload_save_name"></span><span class="up_filename" title="${fileName}">${fileName}</span><span class="btn btn-default btn-sm uploadbtn">上传</span><span class="delfilebtn">&times;</span></div><div class="preview-box"></div><div class="uploadify-progress"><div class="uploadify-progress-bar"></div></div></div>';
        var defaults = {
        ...
        this.each(function() {
			...
			var upload_save_name = option.upload_save_name;
            ...
            fileObj = {
                ...
                funSelect:function(files) {
					// 上传文件个数限制，@nbmxkj 20151117 1:03
					// 个数据限制需要加上已上传文件的个数，
					// 已上传文件个数需要使用 class 名 ：uploadify-queue-item
					// 这需要跟模板设计同步。不然会验证失败的
                	
                	//计算旧附件个数 xiayuq 2015-12-04
                	var formrow=_this.closest('.tml-form-row').next('.tml-form-row');
                	var oldfilenum=0;
                	if(formrow.length){
                		oldfilenum= $('.uploadify-queue-item',formrow).length;
                	}
                	if(option.uploadLimit < files.length + _this.find('.uploadify-queue').find('.uploadify-queue-item').length+oldfilenum){
						alert('仅允许上传文件个数【'+option.uploadLimit+'】');
						return;
					}
...
//                            $html.find(".uploadify-progress").after(num);
                        	var num = '<span class="progressnum"><span class="totalsize">${fileSize}</span></span>'.replace(/\${fileSize}/g, formatFileSize(file.size));
                            $html.find(".up_filename").after(num);
                        }
                        //判断是否显示上传百分比	
                ...
                funDeleteFile:function(index) {
                    for (var i = 0, len = this.fileFilter.length; i < len; i++) {
                        ...
                        if (file.index == index) {
                            ...
                            _this.find("#"+ pId +"_fileuploadfile_"+ index).remove();
                            ...
                            if (xhr.readyState == 4) {
                                ...
                                if (xhr.status == 200) {
                                    ...
                                    if (fileObj.uploadOver) {
                                        ...
                                        setTimeout(function() {_this.find('.up_percent').hide();}, 1000);
                                    }
                                } else {
