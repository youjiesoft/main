...
class Db {
    ...
    // 查询表达式
    protected $selectSql  =     'SELECT%DISTINCT% %FIELD% FROM %TABLE%%JOIN%%WHERE%%EXTENDWHERE%%GROUP%%HAVING%%ORDER%%LIMIT% %UNION%';
    //
    protected $tableUnlog="user_online,mis_system_log";
    protected $tableBackup="mis_auto_wrmve,mis_dynamic_form_propery,mis_dynamic_database_sub";
    /**
     +----------------------------------------------------------
     * 架构函数
     +----------------------------------------------------------
    ...
    protected function debug() {
        ...
        if ( $this->debug ) {
            G('queryEndTime');
            trace($this->queryStr.' [ 执行文件：'.$debugInfo[0]["file"].'，行号:'.$debugInfo[0]["line"].'，运行时间:'.G('queryStartTime','queryEndTime').'s ]','','SQL');
            Log::record($this->queryStr.' [ 执行文件：'.$debugInfo[0]["file"].'，行号:'.$debugInfo[0]["line"].'，运行时间:'.G('queryStartTime','queryEndTime').'s ]',Log::SQL);
        }
    }
    ...
    public function update($data,$options) {
        ...
        $sql   = 'UPDATE '
            ...
            .substr($this->parseWhere(isset($options['extendwhere'])?$options['extendwhere']:''),6)
            .$this->parseOrder(isset($options['order'])?$options['order']:'')
            .$this->parseLimit(isset($options['limit'])?$options['limit']:'')
            .$this->parseLock(isset($options['lock'])?$options['lock']:false);
        //准备两个数组 1.无需数据记录数组；2.数据修改备份之前值数组；可以提高性能
        //写入日志
        $debug=false;
        if($debug!==true && strpos($this->tableUnlog,$options['table'])===false){
        	isset($_SESSION['companyid'])?$companyid=$_SESSION['companyid']:$companyid=0;
        	isset($_SESSION['departmentid'])?$departmentid=$_SESSION['user_dep_id']:$departmentid=0;
        	$sqlresume="";//初始化还原SQL语句
        	if(strpos($this->tableBackup,$options['table'])!==false){
        		$sqlresume=self::dataRoamResumeSql($options['table'],array_keys($data),$this->parseWhere(isset($options['where'])?$options['where']:''),'update');
        	}
        	$sql2='INSERT INTO `mis_system_log` (`name`,`logtype` ,`content` ,`resume`,`createid` ,`actionip`,`createtime`,`companyid`,`departmentid`) VALUES ("'.$options['table'].'","1", "'.$sql.'", "'.$sqlresume.'", '.$_SESSION['authId'].',"'.get_client_ip().'",'.time().','.$companyid.','.$departmentid.')';
        	$this->execute($sql2);
        }                
        $list=$this->query($sql);
        if( $list!==false ){
    ...
    public function delete($options=array()) {
        ...
        //写入日志
        $debug=false;
        if($debug!==true && !strpos($this->unlogTable,$options['table'])){
        	isset($_SESSION['companyid'])?$companyid=$_SESSION['companyid']:$companyid=0;
        	isset($_SESSION['departmentid'])?$departmentid=$_SESSION['user_dep_id']:$departmentid=0;
        	$sqlresume="";//初始化还原SQL语句
        	if(strpos($this->tableBackup,$options['table'])!==false){
        		$sqlresume=self::dataRoamResumeSql($options['table'],"",$this->parseWhere(isset($options['where'])?$options['where']:''),'delete');
        	}
        	$sql2='INSERT INTO `mis_system_log` (`name`,`logtype` ,`content` ,`resume`,`createid` ,`actionip`,`createtime`,`companyid`,`departmentid`) VALUES ("'.$options['table'].'","1", "'.$sql.'", "'.$sqlresume.'", '.$_SESSION['authId'].',"'.get_client_ip().'",'.time().','.$companyid.','.$departmentid.')';
        	$this->execute($sql2);
        }
    ...
    static private function dataRoamResumeSql($targetTable,$data,$where,$operation='update',$type=false){
    	...
    	if($result){
    		...
    		//     	exit;
    		//序列换返回值
    		return base64_encode(serialize($sqlArr));
    	}
    }
    /**
     +----------------------------------------------------------
     ...
     * @return mixed
     +----------------------------------------------------------
     */
    public function select($options=array(),$debugInfo = array()) {
        $this->model  =   $options['model'];
        $sql   = $this->buildSelectSql($options);
    ...
    public function parseSql($sql,$options=array()){
        ...
        $sql   = str_replace(
            array('%TABLE%','%DISTINCT%','%FIELD%','%JOIN%','%WHERE%','%EXTENDWHERE%','%GROUP%','%HAVING%','%ORDER%','%LIMIT%','%UNION%'),
            array(
                $this->parseTable($options['table']),
    ...
    public function setLineNum($line){
    	$this->line  =   $line;
    }
