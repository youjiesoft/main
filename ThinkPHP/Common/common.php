 ...
 * @return Model
  +----------------------------------------------------------
 */
function M($name='', $tablePrefix='',$connection='',$tabletype=0) {
    static $_model = array();
    if(strpos($name,':')) {
    ...
    if (!isset($_model[$name . '_' . $class]))
        $_model[$name . '_' . $class] = new $class($name,$tablePrefix,$connection,$tabletype);
 ...
 * @return void
 */
function traceold($value='[think]',$label='',$level='DEBUG',$record=false) {
	static $_trace =  array();
